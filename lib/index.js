...
let ErrorNoSuchEndpoint = (id) => {
  let error = new Error(format('There is no such endpoint of ID "%s".', id))
  error.code = 'REDIS_GRAPH_NO_SUCH_ENDPOINT'
  return error
}
...
let Graph = (config) => {
  ...
  let assertEdgeEndpointsExist = (edge) => (
    P.join(
  ...
  let createStrict = (spec) => (
    ...
    .then(() => (
      ...
      .then(rejectIfMultiResultsError)
      .tap((results) => {
        if (results[1] && results[2]) publish.createdEdge(spec)
      })
      .return(spec)
    ))
  )
  ...
  let update = (edgeAfter) => {
    ...
    .then(JSON.parse)
    .tap((dataBefore) => {
      if (!equal(dataBefore, edgeAfter.data)) {
        publish.updatedEdge(
          edgeModel(edgeAfter.sid, edgeAfter.pid, dataBefore),
          edgeAfter
        )
      }
    })
  ...
  let asssertEndpointExists = (id) => (
    ...
    .then((exists) => (
      exists
        ? null
        : P.reject(ErrorNoSuchEndpoint(id))
    ))
    .return(id)
  )

  let getFrom = (pid) => (
    asssertEndpointExists(pid)
    .then(() => (
      redis
      .smembers(k({ from: pid }))
  ...
  let getTo = (sid) => (
    asssertEndpointExists(sid)
    .then(() => (
      redis
      .smembers(k({ to: sid }))
      .map((pid) => (
        !pid
          ? P.reject(ErrorNoSuchEndpoint(sid))
          : getBetween({ sid, pid })
      ))
    ))
  )
    ...
    .then((indexes) => (
      !indexes
        ...
        .tap(() => {
          let atomic = redis
          .multi()
          // Delete this endpoints's indexes and edge descriptions.
          .del(
            [ k({ from: id }), k({ to: id }) ]
            ...
            // Remove the endpoint itself.
            .concat([k({ id })])
          )

          // Remove this endpoints's (A) presence in other
          // endpoints' indexes (NS).
